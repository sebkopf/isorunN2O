...
```{r}
df.raw %>% group_by(category) %>% generate_data_table(area, d45, d46)
df.raw %>% group_by(category, name) %>% generate_data_table(area, d45, d46, cutoff = 3)
```
...
Last steps are caculating the background (see `?calculate_background`), calculating concentrations (see `?calculate_concentrations`) and then calibrating $\delta^{15}N$ and $\delta^{18}O$ (see `?calibrate_d15` and `?calibrate_d18`). Note that the background calculation is not currently used for calibration since only double point is implemented. 
...
df.cal %>% 
  ...
  group_by(category) %>% generate_data_table(cutoff = 3, d15.cal, d18.cal)
```
...
# %>% group_by(category, name) %>% generate_data_table(cutoff = 3, d15.raw, d15.cal, d18.raw, d18.cal)
```
